...
// multitouch[][1]: X (normalized 0-1)
// multitouch[][2]: Y
// X and Y coordinates are 0-1.
#define MAXFINGERS 11
float multitouch[MAXFINGERS][3];

void VID_GetTouchFinger(int slot, int *active, float *nx, float *ny)
{
	if (slot < 0 || slot >= MAXFINGERS)
	{
		if (active)
			*active = 0;
		if (nx)
	...
	if (active)
		*active = multitouch[slot][0] ? 1 : 0;
	if (nx)
		*nx = multitouch[slot][1];
	if (ny)
		*ny = multitouch[slot][2];
}
...
static void IN_Move_TouchScreen_SteelStorm(void)
{
	...
	/*
	Enable mouse as a touch device for desktop testing when no SDL fingers are active.
	*/
	if (numfingers == 0 && vid_touchscreen.integer)
	{
		int x, y;
